...
export function documentPath(entry) { return entry.path || entry.name; }
function urlPath(path) { return new URL(path.split('/').map(encodeURIComponent).join('/'), ORIGIN); }
export function importHrefs(body, href) {
  ...
  if (!links.length) throw Error('Import resource ' + href + ' has no JSON link. XML, YAML and embedded base64 imports are not supported.');
...
export function resolveImport(body, href, source, documents) {
  ...
  if (urls.some(url => !['http:', 'https:'].includes(url.protocol))) throw Error('Unsupported import URL scheme: ' + href);
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    const candidates = documents.filter(d => (loose || absolute || !documentPath(d).includes('/')) && documentPath(d).split('/').pop() === name);
    if (candidates.length > 1) throw Error('Ambiguous import: ' + name + '. Open the common parent folder to preserve relative paths.');
    if (candidates.length === 1) return candidates[0];
  }
  throw Error('Missing JSON import: ' + references.join(' or ') + '. Open the common parent folder containing the profile and its sources, or select the source JSON file separately.');
}

export async function readDocuments(files, existing, {folder = false} = {}) {
  const documents = [...existing], errors = []; let added = 0, skipped = 0;
  for (const file of files) {
    const path = file.webkitRelativePath || file.name;
    if (!/\.json$/i.test(file.name)) { skipped++; continue; }
    try {
      ...
      if (!types.length && folder) { skipped++; continue; }
      if (types.length !== 1 || typeof doc[types[0]] !== 'object' || Array.isArray(doc[types[0]])) throw Error('Expected one catalogue or profile object.');
      const entry = {name:file.name, path, doc};
      const index = documents.findIndex(d => documentPath(d) === path);
      if (index < 0) documents.push(entry); else documents[index] = entry;
      added++;
    } catch (e) { errors.push(path + ': ' + e.message); }
  }
  return {documents, added, skipped, errors};
}
